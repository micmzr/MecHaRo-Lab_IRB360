  ...
  const sensor_msgs::msg::JointState::ConstSharedPtr state)
{
  ...
  else if(state_cnt == 4)
  {
    ...
    joints.name[0] = "joint_fix_Axis1";
    joints.name[1] = "joint_fix_Axis2";
    joints.name[2] = "joint_fix_Axis3";
    joints.name[3] = "joint_1";
    joints.name[4] = "joint_1_1_1";
    joints.name[5] = "joint_1_1_2";
    joints.name[6] = "joint_1_1_3";
    joints.name[7] = "joint_1_2_1";
    joints.name[8] = "joint_1_2_2";
    joints.name[9] = "joint_1_2_3";
    joints.name[10] = "joint_2";
    joints.name[11] = "joint_2_1_1";
    joints.name[12] = "joint_2_1_2";
    joints.name[13] = "joint_2_1_3";
    joints.name[14] = "joint_2_2_1";
    joints.name[15] = "joint_2_2_2";
    joints.name[16] = "joint_2_2_3";
    joints.name[17] = "joint_3";
    joints.name[18] = "joint_3_1_1";
    joints.name[19] = "joint_3_1_2";
    joints.name[20] = "joint_3_1_3";
    joints.name[21] = "joint_3_2_1";
    joints.name[22] = "joint_3_2_2";
    joints.name[23] = "joint_3_2_3";
    joints.name[24] = "joint_4";
    joints.name[25] = "joint_4_2";
    joints.name[26] = "joint_4_3";
    joints.name[27] = "joint_4_4";
    joints.name[28] = "joint_4_5";
    joints.name[29] = "joint_4_6";
    joints.name[30] = "joint_4_7";
  }

  if (state_cnt == joints_cnt)
  {
    joints.position[0] = -theta_zero;
    joints.position[1] = -theta_zero;
    joints.position[2] = -theta_zero;
    // Axis - 1
    joints.position[fix_cnt+0] = state->position[fix_cnt+0];
    // Axis - 2
    joints.position[fix_cnt+7] = state->position[fix_cnt+7];
    // Axis - 3
    joints.position[fix_cnt+14] = state->position[fix_cnt+14];
    // Axis - 4
    joints.position[fix_cnt+21] = state->position[fix_cnt+21];
  }
  else if(state_cnt == 4)
  {
    // Axis - 1
    joints.position[fix_cnt+0] = state->position[0];
    // Axis - 2
    joints.position[fix_cnt+7] = state->position[1];
    // Axis - 3
    joints.position[fix_cnt+14] = state->position[2];
    // Axis - 4
    joints.position[fix_cnt+21] = state->position[3];
  }
  else
  {
  ...
  else 
  {
    ...
    ,joints.position[fix_cnt+7],joints.position[fix_cnt+14],joints.position[fix_cnt+21],theta_zero); 
  }
...
bool IRB360FKPublisher::FK(void)
{
  ...
  // ABB RobotStudio version
  joints.position[0] = -theta_zero;
  joints.position[1] = -theta_zero;
  joints.position[2] = -theta_zero;

  double theta1 = joints.position[fix_cnt+0];
  double theta2 = joints.position[fix_cnt+14];
  double theta3 = joints.position[fix_cnt+7]; // reverse order of joints nummbering to the Trossen Delta tutorial
  ...
  double phi_4 = phi_4_1 + joints.position[fix_cnt+21];

  double d_link4 = sqrt( x0*x0 + y0*y0 + pow( z0-z0_0 - l4_0 , 2 ) ) - l4_0;  
  joints.position[fix_cnt+24] = d_link4/1000.l;

  double d_link_4x = sin(phi_4)*y0 + cos(phi_4)*x0; 
  double d_link_4y = -cos(phi_4)*y0 + sin(phi_4)*x0;

  joints.position[fix_cnt+22] = -atan2( d_link_4x , ( - z0 + z0_0 + l4_0 ) );
  joints.position[fix_cnt+23] = -atan2( d_link_4y , sqrt( pow(( - z0 + z0_0 + l4_0 ),2) + d_link_4x*d_link_4x ) );

  joints.position[fix_cnt+25] = joints.position[fix_cnt+23];
  joints.position[fix_cnt+26] = joints.position[fix_cnt+22];
  joints.position[fix_cnt+27] = -joints.position[fix_cnt+21];
  ...
  joints.position[fix_cnt+1] = -joints.position[fix_cnt+0]+theta_zero; 
  joints.position[fix_cnt+2] = -atan2( y0, f/2.l/sqrt3 + rf*cos(theta1) - (x0+e/2.l/sqrt3) ); 
  joints.position[fix_cnt+3] = -alpha_0 + alpha1; 

  joints.position[fix_cnt+4] = joints.position[fix_cnt+1];
  joints.position[fix_cnt+5] = joints.position[fix_cnt+2];
  joints.position[fix_cnt+6] = joints.position[fix_cnt+3];
  ...
  joints.position[fix_cnt+8] = -joints.position[fix_cnt+7]+theta_zero; 
  joints.position[fix_cnt+9] = -atan2( y0_2, f/2.l/sqrt3 + rf*cos(theta3) - (x0_2+e/2.l/sqrt3) ); 
  joints.position[fix_cnt+10] = -alpha_0 + alpha2; 

  joints.position[fix_cnt+11] = joints.position[fix_cnt+8];
  joints.position[fix_cnt+12] = joints.position[fix_cnt+9];
  joints.position[fix_cnt+13] = joints.position[fix_cnt+10];
  ...
  joints.position[fix_cnt+15] = -joints.position[fix_cnt+14]+theta_zero; 
  joints.position[fix_cnt+16] = -atan2( y0_3, f/2.l/sqrt3 + rf*cos(theta2) - (x0_3+e/2.l/sqrt3) ); 
  joints.position[fix_cnt+17] = -alpha_0 + alpha3; 

  joints.position[fix_cnt+18] = joints.position[fix_cnt+15];
  joints.position[fix_cnt+19] = joints.position[fix_cnt+16];
  joints.position[fix_cnt+20] = joints.position[fix_cnt+17];
